...
const AddUser = () => {
    ...
    async function handleAddUser(){
        if(!name && !password && !stationID && !email){
            toast({
        ...
        const res=await axios.post('http://localhost:8080/register/siding/user', {
            email:email,
            password:password,
            stationID:stationID,
            name:name,
          ...
          <div>
            <Card variant="filled" sx={{ boxShadow: "0px 0px 0px 10px white" }}>
              <CardBody>
                ...
                <CardHeader>
                  <Heading size="md">
                    <div className="font-ubuntu">Enter Station ID</div>
                  </Heading>
                </CardHeader>
                <Text>
                  <div className="flex flex-row">
